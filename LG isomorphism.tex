...
%\amandain{Should we say explicitly something about BV mirror symmetry here?}
\end{abstract}
...
\end{rem} %\amandain{Than what?  The LG model inspired by BV construction?} 
...
%\nathan{We switched somehow from $p$ and $q$ to $m$ and $n$, and I think we mean just the polynomials and not the volume forms? Is that right?}
It is clear that $pq$ is a scalar multiple of $\Hess(W)$.We want to show that $g=h^{-1}$. %$tw^{-1}(g') = (tw^{-1}(h'))^{-1}$.
...
%\nathanin{I restructured  this. The text is more dense now, but also a quicker read. What do you think?We could make list out of the input still, if it would make it look better there.}
%\begin{itemize}
