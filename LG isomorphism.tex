...
\begin{rem}
In fact what we call a BVLG model is slightly more general. \amandain{Than what?  The LG model inspired by BV construction?} If we take for a group $G$ a group that can be written as a product of subgroups $G=G_1\times G_2$ with $G_1\subset \SL_{W_1}$ and $ G_2\subset \SL_{W_2}$, then we recover the construction of Borcea--Voisin. If, on the other hand, one considers a group $G$ that is not such a product, the corresponding geometry is no longer a variety of Borcea--Voisin type, but rather an orbifold, whose crepant resolution is the Nakamura--Hilbert scheme of $\widetilde{\s G}$-regular orbits of $X_1\times X_2$ (cf. \cite{Na}), as was also mentioned in the introduction.  
\end{rem}
...
In the notation just described, $2\jw_{W_1}=(0,2\jw_x,0,0)$, and so $(2\jw_x,0)\in \tw{G}$. Since $p'$ is fixed by $\tw{G}$, it is fixed by $(2\jw_x,0)$, which acts trivially on the $y$ variables, and so $(2\jw_x,0)$ fixes $p_x$. %A similar statement is true for $2\jw_y$. 
...
g_0 A_W h \equiv 0\pmod \ZZ.
$
%But recall $\alpha A_1\jw_x\equiv \tfrac 12\pmod \ZZ$, hence
%\[
...
k_{{m},{g}}\fjrw{m'dx_0 \wedge dy_0}{(0, \alpha, 0, \beta)} & \text{ otherwise}, \end{array}\right.
 \end{array}
 \end{equation}
 where $\jw_{x,B}$ is the exponential grading operator associated with $f_1^T$. %We make the following two useful remarks to characterize whether $\fjrw{m}{g}\in \sB_{W^T,\s{G^T}}$. 
...
%In other words, $\kappa_{m,g} =1$ if $ \fjrw{m}{g} \in \sB_{W^T, \s{G^T}}$ and $\kappa_{m,g}\in \{1/2,2\}$ otherwise. 
%Since $m$ only determines if $\kappa_{m,g}=1$ or not, we can henceforth drop the subscript $g$ with no confusion, i.e. $\kappa_g=\kappa_{m,g}$. 
...
\]
since $\sigma g \sigma h = gh$.  %Further, $\varphi( \fjrw{mn}{ gh}) = \fjrw{mn}{ gh}$, by Remark \ref{rem_action}. 
Thus, to verify that  
$
\varphi(\fjrw{m}{ g}) \star_{\sB_2}  \varphi( \fjrw{n}{ h}) =\varphi(\fjrw{m}{ g} \star_{\sB_1}   \fjrw{n}{ h})$, % =  \varphi(\gamma_{ g, h} \fjrw{mn}{ gh})\\
...
\begin{enumerate}
\item  $g$ and $h$ are both of the form $(0, \alpha, 0, \beta)$. \\
Here, $k_g = k_h = 1/2$. Suppose that $I_g \cup I_h \cup I_{gh} = \{x_0, \ldots, x_m,y_0,\ldots,y_n\}$. 
Lemma \autoref{lem:gamma_relations} gives 
\[
\gamma_{g,h}  = \gamma_{g,\sigma h} 
%\frac{\Hess (W_{gh})\mu_{g \cap h}}{\Hess (W_{g\cap h})\mu_{gh}} 
%=\frac{\Hess (W_{gh})\mu_{\sigma g \cap \sigma h}}{4\Hess (W_{\sigma g\cap \sigma h})\mu_{gh}} 
...
\[
\varphi(\fjrw{m}{g} \star_{\sB_1} \fjrw{n}{h}) = \varphi(\gamma_{g,h} \fjrw{mn}{gh}) = \gamma_{g,h}k_{gh} \fjrw{\widetilde{mn}} {\sigma gh},
\]
and, 
\[
\varphi(\fjrw{m}{g}) \star_{\sB_2} \varphi(\fjrw{n}{h}) = \fjrw{m}{g} \star_{\sB_2} k_h \fjrw{\tilde n}{\sigma h} = \gamma_{g,\sigma h}k_{h} \fjrw{\widetilde{mn}} {\sigma gh}.
\]
...
\begin{enumerate}
\item $g$ and $h$ are both of the form $(0, \alpha, 0, \beta)$ ($k_h =k_{gh}= 1/2$).  From Lemma \ref{lem:gamma_relations}, 
$
\gamma_{g,h}  %=
%\frac{\Hess (W_{gh})\mu_{g \cap h}}{\Hess (W_{g\cap h})\mu_{gh}} 
...
\[
\left(\prod_{i=1}^r \mathbb{P}\left(w_0^{(i)}, \ldots, w_{k_1}^{(i)}\right)\right)/\mathbb{Z}_2 \to 
\mathbb{P}\left(\left(\prod_{i\ne 1}w_0^{(i)}\right)w_1^{(1)}, \ldots, \left(\prod_{i\ne j}w_0^{(i)}\right)w_1^{(j)}, \ldots, \left(\prod_{i\ne r}w_0^{(i)}\right)w_{k_r}^{(r)}\right),
\] 
where $\mathbb{Z}_2$ acts by negating the first coordinate of each factor, and sends 
\[
\left(\prod_{i=1}^r \{f_i = 0\}\right)/\mathbb{Z}_2$$ to $$\{ f_1 = f_2 = \ldots = f_r\}.
\]
